$("#table_cmd_ipx800_analogique").delegate(".listEquipementInfo", 'click', function () {
    var el = $(this);
   ...
   if (!isset(_cmd)) {
    ...
    if (init(_cmd.type) == 'info') {
        ...
        if (init(_cmd.logicalId) == 'reel') {
			...
			tr += '<a class="btn btn-default cursor listEquipementInfo" data-input="calcul" style="margin-top : 5px;"><i class="fas fa-list-alt "></i> {{Rechercher équipement}}</a>';
...
$('#bt_configPush').on('click', function() {
    $('#md_modal').dialog({title: "{{Configurer l'IPX pour avoir le retour des etats}}"});
    $('#md_modal').load('index.php?v=d&plugin=ipx800&modal=ipx800.configpush&id=' + $('.eqLogicAttr[data-l1key=id]').value()).dialog('open');
});
...
$('#bt_ApplyconfigPush').on('click', function() {
    ...
    $.ajax({// fonction permettant de faire de l'ajax
        ...
        url: "plugins/ipx800/core/ajax/ipx800.ajax.php", // url du fichier php
        data: {
            action: "configPush",
			id: $('.li_eqLogic.active').attr('data-eqLogic_id'),
